...
export default function Dashboard() {
  ...
  useEffect(() => {
    api.get('/api/dashboard/summary').then(({ data }) => {
      setSummary(data)
      setLoading(false)
    })
  }, [])
  ...
  if (loading || !summary) {
    return <div className="loading-dots">Loading dashboard…</div>
  }

  const statusCounts = summary.submissionStatusByMember.reduce((acc, s) => {
    acc[s.status] = (acc[s.status] || 0) + 1
    return acc
  }, {})
  const statusBarData = Object.entries(statusCounts).map(([status, count]) => ({ status, count }))
